...
#include "entity/libglsimplify_sphere.h"
...
int test_sphere(int argc, char **argv, int width, int height)
{
    ...
    do
    {
        ...
        do
        {
            gl_simplify::entity::SpherePtr sphere = CreateSphere();

            sphere->SetRadius(1);
            sphere->SetLatitudeSegments(segments);
            sphere->SetLongitudeSegments(segments);
            sphere->Create();
            sphere->Attatch(wood);
